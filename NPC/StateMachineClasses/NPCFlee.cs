...
public partial class NPCFlee : NPCState
{
	...
	public Vector3 FindFleeVector() {					//TLDR; Return vector from threat to NPC
		threatPosition = NPC.GetFocus().GlobalPosition;	//by constantly pulling the threat we have threat updating taking place in the character node & have this node react dynamically
	...
	public void TravelFleePoint(Vector3 fleeVect)
	{
		...
		direction.Y = 0;
		NPC.Velocity = direction * NPC.GetRunSpeed();
	}
